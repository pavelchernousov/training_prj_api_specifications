Учебный проект **Спецификация API**

на примере **Обмен данными между CRM и складской системой**

#### 📌 **Описание задачи**  
Компания использует **CRM** для работы с клиентами и **складскую систему** для учета товаров. Нужно автоматизировать обмен данными, чтобы:  
1. **Обновлять информацию о наличии товаров в CRM** после изменений на складе.  
2. **Передавать заказы из CRM в складскую систему** для их резервирования и отгрузки.  

#### 🎯 **Цели интеграции**  
- Автоматическое обновление данных о товарах.  
- Минимизация ручного ввода данных.  
- Исключение ситуаций, когда менеджеры продают товар, которого нет на складе.
- 
**План работы** 
- *Определяем сущности, участвующие в процессе*
- *Строим диаграмму последовательности с помощью PlantUML*
- *Знакомимся с HTTP и REST-like API*
- *Выявляем ресурсы*
- *Определяем методы*
- *Описание спецификации REST API*
- *Описываем спецификацию/контракт API в табличном виде*
- *Узнаём, чем отличаются идентификация, аутентификация и авторизация*
- *Рассматриваем версионирование API*
- *Обсуждаем нефункциональные требования*
- *Рассматриваем примеры API*
---
### 🔹 **Шаг 1: Разбираем бизнес-процессы, требующие интеграции**  

В нашей задаче участвуют две системы:  
- **CRM** (работают менеджеры, создают заказы).  
- **Складская система** (учет товаров, отгрузка, резервация).  

#### 🔥 **Ключевые бизнес-процессы для интеграции**  
1. **Обновление остатков товаров в CRM**  
   - Менеджеры в CRM должны видеть актуальные остатки.  
   - Когда товар поступает или списывается на складе, информация обновляется.  

2. **Передача заказа со склада в CRM**  
   - Менеджер создает заказ в CRM.  
   - CRM отправляет заказ в складскую систему.  
   - Если товар есть, он резервируется.  
   - Если товара нет, CRM получает уведомление и информирует менеджера.  

3. **Подтверждение отгрузки заказа**  
   - Когда заказ отгружен со склада, CRM получает подтверждение.  
   - Менеджер видит статус ("Отгружено") и может уведомить клиента.  

#### 📌 **Дополнительные нюансы**  
- Ошибки при передаче данных (например, если склад недоступен).  
- Возможность частичной отгрузки заказа.  
- Версионирование API для будущих обновлений.  
---  
### 🔹 **Шаг 2: Определяем сущности, участвующие в процессе**  

При интеграции CRM и складской системы основными **сущностями** (объектами данных) будут:  

1. **Товар (Product)**  
   - `id` — уникальный идентификатор товара.  
   - `name` — название товара.  
   - `sku` — артикул.  
   - `quantity` — количество товара на складе.  
   - `price` — цена.  
   - `updated_at` — дата последнего обновления.  

2. **Заказ (Order)**  
   - `id` — уникальный идентификатор заказа.  
   - `customer_id` — ID клиента в CRM.  
   - `items` — список товаров в заказе (id товара, количество).  
   - `total_price` — общая сумма заказа.  
   - `status` — статус заказа (например, "Новый", "В резерве", "Отгружен").  
   - `created_at` — дата создания.  

3. **Клиент (Customer)** (если CRM передает данные о клиенте)  
   - `id` — уникальный ID клиента.  
   - `name` — имя клиента.  
   - `email` — почта.  
   - `phone` — телефон.  

4. **Уведомление об изменении статуса заказа (OrderStatusUpdate)**  
   - `order_id` — ID заказа.  
   - `status` — новый статус заказа.  
   - `updated_at` — время обновления.  

---
### 🔹 **Шаг 6: Выявляем ресурсы для API**  

В нашей интеграции CRM ↔ Складская система понадобятся три ключевых ресурса:  

| **Ресурс**  | **Описание**  | **Пример URL** |
|------------|--------------|---------------|
| `products`  | Товары на складе  | `/products` |
| `orders`    | Заказы, передаваемые на склад  | `/orders` |
| `order-status` | Статус заказов | `/order-status` |

---

### 📌 **Разбираем ресурсы по деталям**  

1️⃣ **Ресурс: `products` (товары на складе)**  
CRM должна получать актуальную информацию о товарах.  

✅ **Методы API:**  
| **Метод** | **Описание** | **Пример запроса** |
|-----------|-------------|------------------|
| `GET /products` | Получить список товаров | `GET /products` |
| `GET /products/{id}` | Получить информацию о конкретном товаре | `GET /products/5` |

📌 **Пример ответа:**  
```json
[
    {
        "id": 1,
        "name": "Ноутбук",
        "sku": "LAP123",
        "quantity": 15,
        "price": 80000
    },
    {
        "id": 2,
        "name": "Мышь беспроводная",
        "sku": "MOUSE456",
        "quantity": 50,
        "price": 1500
    }
]
```

---

2️⃣ **Ресурс: `orders` (создание заказов в складской системе)**  
CRM передает заказ на склад, чтобы товар зарезервировали.  

✅ **Методы API:**  
| **Метод** | **Описание** | **Пример запроса** |
|-----------|-------------|------------------|
| `POST /orders` | Создать новый заказ | `POST /orders` |
| `GET /orders/{id}` | Получить детали заказа | `GET /orders/10` |

📌 **Пример запроса на создание заказа:**  
```json
{
    "customer_id": 123,
    "items": [
        { "product_id": 1, "quantity": 1 },
        { "product_id": 2, "quantity": 2 }
    ],
    "total_price": 83000
}
```

📌 **Пример ответа (создан заказ №10):**  
```json
{
    "order_id": 10,
    "status": "reserved"
}
```

---

3️⃣ **Ресурс: `order-status` (обновление статуса заказа)**  
Когда заказ обработан складом, CRM должна узнать об изменении статуса.  

✅ **Методы API:**  
| **Метод** | **Описание** | **Пример запроса** |
|-----------|-------------|------------------|
| `GET /order-status/{order_id}` | Получить статус заказа | `GET /order-status/10` |
| `PATCH /order-status/{order_id}` | Обновить статус заказа | `PATCH /order-status/10` |

📌 **Пример запроса на обновление статуса заказа:**  
```json
{
    "status": "shipped"
}
```

📌 **Пример ответа:**  
```json
{
    "order_id": 10,
    "status": "shipped",
    "updated_at": "2025-02-14T12:00:00Z"
}
```

---

### 🔥 **Что дальше?**  
Теперь мы **определили ресурсы**, и можно переходить к **описанию методов API**.  

Ты хочешь попробовать описать один метод сам, или разберем вместе? 😊
### 🔹 **Шаг 7: Определяем методы API**  

Теперь детально опишем основные методы API. Мы уже знаем, какие ресурсы нам нужны (`products`, `orders`, `order-status`), теперь разберем их методы.  

#### 📌 **Метод 1: Получение списка товаров**  
- **Описание**: CRM запрашивает список товаров на складе.  
- **HTTP-метод**: `GET`  
- **URL**: `/products`  

✅ **Пример запроса:**  
```http
GET /products
Host: warehouse.example.com
```

✅ **Пример ответа (JSON):**  
```json
[
    {
        "id": 1,
        "name": "Ноутбук",
        "sku": "LAP123",
        "quantity": 15,
        "price": 80000
    },
    {
        "id": 2,
        "name": "Мышь беспроводная",
        "sku": "MOUSE456",
        "quantity": 50,
        "price": 1500
    }
]
```

---

#### 📌 **Метод 2: Создание заказа**  
- **Описание**: CRM передает заказ в складскую систему.  
- **HTTP-метод**: `POST`  
- **URL**: `/orders`  
- **Тело запроса (Body):**  
  - `customer_id` — ID клиента  
  - `items` — список товаров (ID товара и количество)  
  - `total_price` — сумма заказа  

✅ **Пример запроса:**  
```http
POST /orders
Host: warehouse.example.com
Content-Type: application/json
```

📌 **Тело запроса (JSON):**  
```json
{
    "customer_id": 123,
    "items": [
        { "product_id": 1, "quantity": 1 },
        { "product_id": 2, "quantity": 2 }
    ],
    "total_price": 83000
}
```

✅ **Пример ответа:**  
```json
{
    "order_id": 10,
    "status": "reserved"
}
```

📌 **Коды ответа:**  
| Код | Значение |
|-----|---------|
| `201 Created` | Заказ успешно создан |
| `400 Bad Request` | Ошибка в данных запроса |
| `500 Internal Server Error` | Ошибка сервера |

---

#### 📌 **Метод 3: Обновление статуса заказа**  
- **Описание**: Складская система обновляет статус заказа (например, «Отгружен»).  
- **HTTP-метод**: `PATCH`  
- **URL**: `/order-status/{order_id}`  
- **Тело запроса (Body):**  
  - `status` — новый статус заказа  

✅ **Пример запроса:**  
```http
PATCH /order-status/10
Host: warehouse.example.com
Content-Type: application/json
```

📌 **Тело запроса (JSON):**  
```json
{
    "status": "shipped"
}
```

✅ **Пример ответа:**  
```json
{
    "order_id": 10,
    "status": "shipped",
    "updated_at": "2025-02-14T12:00:00Z"
}
```

📌 **Коды ответа:**  
| Код | Значение |
|-----|---------|
| `200 OK` | Статус успешно обновлен |
| `400 Bad Request` | Ошибка в запросе |
| `404 Not Found` | Заказ не найден |

---

### 🔥 **Что дальше?**  
Теперь у нас есть описание методов API. Следующий шаг — **описание спецификации API в табличном виде**.  

Как думаешь, удобно ли представить все методы в виде таблицы, или разобрать это более детально? 😊

Хороший вопрос! В методе **`GET /products`** не требуется заголовок `Content-Type`, потому что:  

1. **GET-запрос не содержит тело**  
   - `Content-Type` указывает, в каком формате отправляются данные в теле запроса.  
   - В `GET`-запросах **тела нет**, поэтому этот заголовок не нужен.  

2. **Сервер сам указывает формат ответа**  
   - Вместо `Content-Type` в запросе, клиент может использовать заголовок `Accept`, чтобы указать предпочтительный формат ответа.  
   - Пример:  
     ```http
     GET /products
     Host: warehouse.example.com
     Accept: application/json
     ```
   - А сервер в ответе уже укажет `Content-Type`, например:  
     ```http
     HTTP/1.1 200 OK
     Content-Type: application/json
     ```

---

### ❓ **Когда `Content-Type` нужен?**  
`Content-Type` обязателен в **POST** и **PATCH** запросах, потому что клиент отправляет данные в теле запроса. Например:  
```http
POST /orders
Host: warehouse.example.com
Content-Type: application/json
```
```json
{
    "customer_id": 123,
    "items": [ { "product_id": 1, "quantity": 2 } ]
}
```

---

Давай оформим API в удобную таблицу!  

---

### 🔹 **Спецификация API для интеграции CRM ↔ Склад**
| **Метод**  | **URL**  | **Описание**  | **Тело запроса**  | **Ответ**  | **Коды ответа** |
|------------|----------|--------------|------------------|------------|----------------|
| **GET** | `/products` | Получить список товаров на складе | ❌ | JSON-массив товаров | `200 OK`, `500 Internal Server Error` |
| **GET** | `/products/{id}` | Получить данные о конкретном товаре | ❌ | JSON-объект товара | `200 OK`, `404 Not Found` |
| **POST** | `/orders` | Создать новый заказ | JSON с `customer_id`, `items`, `total_price` | JSON с `order_id` и `status` | `201 Created`, `400 Bad Request`, `500 Internal Server Error` |
| **GET** | `/orders/{id}` | Получить информацию о заказе | ❌ | JSON с деталями заказа | `200 OK`, `404 Not Found` |
| **PATCH** | `/order-status/{order_id}` | Обновить статус заказа | JSON с `status` | JSON с обновленным статусом | `200 OK`, `400 Bad Request`, `404 Not Found` |

---

### 📌 **Примеры запросов**
1️⃣ **Получение списка товаров**  
```http
GET /products
Host: warehouse.example.com
Accept: application/json
```
**Ответ:**  
```json
[
    {
        "id": 1,
        "name": "Ноутбук",
        "sku": "LAP123",
        "quantity": 15,
        "price": 80000
    }
]
```

2️⃣ **Создание заказа**  
```http
POST /orders
Host: warehouse.example.com
Content-Type: application/json
```
**Тело запроса:**  
```json
{
    "customer_id": 123,
    "items": [
        { "product_id": 1, "quantity": 1 }
    ],
    "total_price": 80000
}
```
**Ответ:**  
```json
{
    "order_id": 10,
    "status": "reserved"
}
```

---

### 🔥 **Что дальше?**  
Следующий шаг — **описываем спецификацию/контракт API в табличном виде (Swagger, OpenAPI, Markdown)**.  

Ты хочешь научиться описывать API в **Swagger (OpenAPI)** или использовать **Markdown** для документации? 😊

Отлично! Давай разберем два популярных способа описания API:  

1. **Swagger / OpenAPI (YAML)** – формальный стандарт для API-документации.  
2. **Markdown** – простой способ документирования API в текстовом формате.  

---

## 🔹 **Способ 1: Описание API в Swagger / OpenAPI**
**OpenAPI** – это формат описания API в **YAML** или **JSON**, который можно использовать в Swagger UI, Postman и других инструментах.  

### 📌 **Пример OpenAPI-документации**  
```yaml
openapi: 3.0.0
info:
  title: CRM-Warehouse API
  description: API для интеграции CRM и складской системы
  version: 1.0.0

servers:
  - url: https://warehouse.example.com

paths:
  /products:
    get:
      summary: Получить список товаров
      responses:
        "200":
          description: Успешный ответ
          content:
            application/json:
              schema:
                type: array
                items:
                  $ref: "#/components/schemas/Product"
        "500":
          description: Ошибка сервера

  /orders:
    post:
      summary: Создать новый заказ
      requestBody:
        required: true
        content:
          application/json:
            schema:
              $ref: "#/components/schemas/Order"
      responses:
        "201":
          description: Заказ создан
          content:
            application/json:
              schema:
                $ref: "#/components/schemas/OrderResponse"
        "400":
          description: Ошибка запроса
        "500":
          description: Ошибка сервера

components:
  schemas:
    Product:
      type: object
      properties:
        id:
          type: integer
          example: 1
        name:
          type: string
          example: "Ноутбук"
        sku:
          type: string
          example: "LAP123"
        quantity:
          type: integer
          example: 15
        price:
          type: number
          example: 80000

    Order:
      type: object
      properties:
        customer_id:
          type: integer
          example: 123
        items:
          type: array
          items:
            type: object
            properties:
              product_id:
                type: integer
                example: 1
              quantity:
                type: integer
                example: 2
        total_price:
          type: number
          example: 83000

    OrderResponse:
      type: object
      properties:
        order_id:
          type: integer
          example: 10
        status:
          type: string
          example: "reserved"
```
📌 **Что дальше?**  
Этот файл можно загрузить в **Swagger Editor** ([editor.swagger.io](https://editor.swagger.io/)) и сразу получить интерактивную документацию API.

---

## 🔹 **Способ 2: Описание API в Markdown**  
Markdown – удобен для документирования API в README-файлах, в Confluence или на GitHub.

### 📌 **Пример Markdown-документации**
```markdown
# CRM-Warehouse API

## 📌 Получить список товаров
**GET /products**
- **Описание:** Получение списка товаров на складе.
- **Ответ (200 OK):**
```json
[
    {
        "id": 1,
        "name": "Ноутбук",
        "sku": "LAP123",
        "quantity": 15,
        "price": 80000
    }
]
```
---

## 📌 Создать заказ
**POST /orders**
- **Описание:** Передача заказа в складскую систему.
- **Тело запроса:**
```json
{
    "customer_id": 123,
    "items": [
        { "product_id": 1, "quantity": 1 }
    ],
    "total_price": 80000
}
```
- **Ответ (201 Created):**
```json
{
    "order_id": 10,
    "status": "reserved"
}
```
---

## 📌 Обновление статуса заказа
**PATCH /order-status/{order_id}**
- **Описание:** Изменение статуса заказа.
- **Тело запроса:**
```json
{
    "status": "shipped"
}
```
- **Ответ (200 OK):**
```json
{
    "order_id": 10,
    "status": "shipped",
    "updated_at": "2025-02-14T12:00:00Z"
}
```
```
📌 **Что дальше?**  
- Markdown-файл можно хранить в **GitHub**, **Confluence** или **Notion**.  
- Swagger-документ можно использовать для **автоматической генерации API-клиента**.  

---

### 🔥 **Какой формат тебе больше нравится – Swagger или Markdown? Или попробовать оба?** 😊
